...
void ForestSurvival::loadForest(size_t num_trees, std::vector<std::vector<std::vector<size_t>> >& forest_child_nodeIDs,
  ...
  for (size_t i = 0; i < num_trees; ++i) {
    trees.push_back(
        make_unique_ranger<TreeSurvival>(forest_child_nodeIDs[i], forest_split_varIDs[i], forest_split_values[i],
...
void ForestSurvival::growInternal() {
  trees.reserve(num_trees);
  for (size_t i = 0; i < num_trees; ++i) {
    trees.push_back(make_unique_ranger<TreeSurvival>(&unique_timepoints, &response_timepointIDs));
  }
}
...
void ForestSurvival::writeImageMask() {
  //TODO make this work with predall

  //TODO make this work with survival trees if there is anything
  //special about them
...
void ForestSurvival::loadFromFileInternal(std::ifstream& infile) {
  ...
  for (size_t i = 0; i < num_trees; ++i) {
    ...
    // Create tree
    trees.push_back(
        make_unique_ranger<TreeSurvival>(child_nodeIDs, split_varIDs, split_values, chf, &unique_timepoints,
